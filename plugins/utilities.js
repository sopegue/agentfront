...
const utility = {
  ...
  dating(date) {
    if (Math.trunc(this.inSeconds(date, new Date())) > 60) {
      if (Math.trunc(this.inMinutes(date, new Date())) > 60) {
        if (Math.trunc(this.inHours(date, new Date())) > 24) {
          if (Math.trunc(this.inDays(date, new Date())) > 7) {
            if (Math.trunc(this.inWeeks(date, new Date())) > 4) {
              if (Math.trunc(this.inMonths(date, new Date())) > 12) {
                return (
                  'Il y a ' +
