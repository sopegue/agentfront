        ...
        <div class="dropdown-content bg-white border">
          ...
          <div class="h-40maxs overflow-y-auto aside">
            <a
              ...
              @click="setcur(element.city)"
            >
              <span
                class="size-125"
                :class="{ 'font-semibold': currency === element.city }"
                >{{ element.city }}</span
              >
            </a>
          </div>
...
export default {
  ...
  computed: {
    language() {
      return this.content.filter((x) =>
        x.city.toLowerCase().includes(this.searching.toLowerCase())
      )
    },
  },
  watch: {
  ...
  mounted() {
    if (this.first === '') this.currency = this.content[0].city
