<template>
  <div v-if="dataOk" class="py-5 px-6 bg-white">
    ...
    <div class="pt-3 pb-20" :class="{ 'px-10': !modaled && size > 940 }">
      <h4 class="logo-color size-16 font-semibold mb-5 text-c border-b pb-1">
        Modification de votre propriété
      </h4>
        ...
        <div>
          ...
          <div class="flex flex-col space-y-4">
            <div>
              ...
              <typeprop
                :first="property.data.property.type"
                :content="[
                  'Studio',
                  'Maison',
                  'Appartement',
        ...
        <div>
          ...
          <div class="flex flex-col space-y-4">
            ...
            <div>
              ...
              <typeprop
                :first="property.data.adresse.ville"
        ...
        <div>
          ...
          <div class="flex flex-col space-y-4">
            <div>
              ...
              <typeprop
                :first="property.data.property.proposition"
                :content="[
                  'Vente totale',
                  'Vente partielle',
                  'Location totale',
            ...
            <div v-show="propos.includes('partielle')" class="appearZ">
              ...
              <typeprop
                :first="property.data.property.percentage_part.toString() + '%'"
            ...
            <div v-show="propos.includes('Location')" class="appearZ">
              ...
              <typeprop
                :first="property.data.property.location_freq"
                :content="[
                  'Par jour',
                  'Par semaine',
                  'Par mois',
            ...
            <div :class="{ noclick: prixmin.length > 0 || prixmax.length > 0 }">
              ...
              <div class="flex align-center space-x-2 pt-1.5">
                <input
                  ...
                  type="checkbox"
                  value="yes"
                  class="border rounded no-outlines size-14 outline-none"
                />
...
export default {
  ...
  methods: {
    ...
    fill() {
      ...
      this.taille =
        this.property.data.property.taille.toString() === '0'
          ? ''
          : this.property.data.property.taille.toString()
      this.adresse = this.property.data.adresse.adresse
      this.ville = this.property.data.adresse.ville
      this.cp =
        this.property.data.adresse.cp === null
          ? ''
          : this.property.data.adresse.cp
      this.propos = this.property.data.property.proposition
      this.partielle = this.property.data.property.percentage_part
      this.typeloc = this.property.data.property.location_freq

      this.prix =
        this.property.data.property.price_fixed.toString() === '0'
          ? ''
          : this.property.data.property.price_fixed.toString()

      this.negociable =
        this.property.data.property.negociable === 'yes' ? 'yes' : 'no'
      this.negoc =
      ...
      if (this.property.data.links !== null) {
        this.links.yt =
          this.property.data.links.yt === null
            ? ''
            : this.property.data.links.yt

        this.links.tiktok =
          this.property.data.links.tiktok === null
            ? ''
            : this.property.data.links.tiktok

        this.links.insta =
          this.property.data.links.insta === null
            ? ''
      ...
      this.pieces =
        this.property.data.property.bed === 0
          ? ''
          : this.property.data.property.bed.toString()

      this.bath =
        this.property.data.property.bath === 0
          ? ''
          : this.property.data.property.bath.toString()

      this.garage =
        this.property.data.property.garage === 0
          ? ''
          : this.property.data.property.garage.toString()

      for (let index = 0; index < this.property.data.options.length; index++) {
        const option = this.property.data.options[index]
        if (option.options.type === 'indoor') {
          this.indoor.push(option.options.title)
        }
        if (option.options.type === 'outdoor') {
          this.outdoor.push(option.options.title)
        }
        if (option.options.type === 'energie') {
          this.energy.push(option.options.title)
        }
      }
    },
    ...
    sameImg(val, parent) {
      for (let index = 0; index < val.length; index++) {
        const img = val[index]
        if (parent.includes(img.url)) return img.url
      }
    },
    changeactive(val) {
    ...
    delpic() {
      ...
      if (this.images.length > 0) {
        ...
        if (index > 0) {
          this.active = index - 1
          this.image = this.images[this.active]
          document.getElementById('vidsource').src = this.image
        } else if (index === 0) {
          this.active = 0
          if (this.images.length > 0) {
